...
public class ListTest {
	...
	public void testAdd() {
		...
		Course temp = (Course) coursesToSelect.get(0);   //输出对象类型强转为Course类型
		System.out.println("添加了课程：" + temp.id + ":" + temp.name);
		
		Course cr2 = new Course("2", "C语言");
		coursesToSelect.add(0, cr2);              //把cr2放到第一位
		Course temp2 = (Course) coursesToSelect.get(0);
		System.out.println("添加了课程：" + temp2.id + ":" + temp2.name);
		
		coursesToSelect.add(cr1);      //重复添加    list中的元素可以重复
...
//		coursesToSelect.add(4, cr3);
		...
		coursesToSelect.addAll(Arrays.asList(course));    // 把数组转变为list 然后传入
